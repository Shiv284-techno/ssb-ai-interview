 ...
 *
 *   - A candidate has at most ONE unsettled attempt. Without that, a candidate
 *     sits the paper, discards it, and starts a fresh twenty-five minutes, and
 *     the time limit means nothing across attempts.
 *
...
export interface OirAttemptStore {
  /**
   * Persists a new attempt. Must reject if the candidate already holds an
   * unsettled one — the check belongs in the store because only the store can
   * make it atomic.
   */
